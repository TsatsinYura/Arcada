...
class MainScr(Screen):
  def __init__(self, **kwargs):
      super().__init__(**kwargs)
      vl = BoxLayout(orientation='vertical', padding=23, spacing=25)
      hl = BoxLayout(padding=25, spacing=25)
      txt = Label(text= 'Выбери экран', color = 'orange')
      vl.add_widget(ScrButton(self, direction='down', goal='first', text="1", color = 'red'))
...
class FirstScr(Screen):
  def __init__(self, **kwargs):
      super().__init__(**kwargs)
      vl = BoxLayout(orientation='vertical', size_hint=(.12, .10), pos_hint={'center_x': 0.5, 'center_y': 0.5})
      btn = Button(text= 'Выбор: 1', size_hint=(.5, .5), pos_hint={'left': 0.7})
      btn_back = ScrButton(self, direction='up', goal='main', text="Назад", size_hint=(.5, 1), pos_hint={'right': 1}, color = (0.225,0.999,0))
      vl.add_widget(btn)
